...
bool isHTMLCodeCorrect(QStringList htmlCode)
{
    ...
    // Проверить, содержит ли текст html-код...
    QRegExp rxHtmlTag("<html>(.*)</html>|<HTML>(.*)</HTML>|<html ([^<]*)>(.*)</html>|<HTML ([^<]*)>(.*)</HTML>");

    // Считать начальной позицией поиска начало строки с html-кодом
    int lastPos = 0;
    ...
    QRegExp rxOpenTags("<h[1-6]>([^<]*)|<H[1-6]>([^<]*)|<h[1-6]( +)([^<]*)>([^<]*)|<H[1-6]( +)([^<]*)>([^<]*)");

    // Считать начальной позицией поиска парного тега заголовка начало строки с html-кодом
    lastPos = 0;

    // Считать начальной позицией поиска открывающегося тега заголовка начало строки с html-кодом
    int pos = 0;

    // Пока в строке с html-кодом найден парный тег заголовка и найден открывающийся тег заголовка
    while((lastPos = rxHeaderTags.indexIn(htmlCodeInOneStr, lastPos)) != -1 && (pos = rxOpenTags.indexIn(htmlCodeInOneStr, pos)) != -1)
    {
        ...
        // Считать начальной позицией для поиска позицию, следующую за найденным открывающимся тегом заголовка
        pos += rxOpenTags.matchedLength();
    }
